...
-- github авто обновляющийся текст.
...
if main_window_state.v then 
    ...
    if imgui.Button(fa.ICON_TERMINAL .. u8" Команды", imgui.ImVec2(100, 50)) then
     ...
     if imgui.Button(fa.ICON_NEWSPAPER_O .. u8" Новости", imgui.ImVec2(100, 50)) then
        act = 5
     end
    ...
    if act == 1 then
        ...
        imgui.Text(u8"Ваша версия " .. script_version .. ".") 
        imgui.Text(update_text_github)
        imgui.SameLine()
        
        if imgui.Button(fa.ICON_CLOUD_DOWNLOAD .. u8" Обновить скрипт", imgui.ImVec2(159,25)) then
            os.execute('explorer "https://www.blast.hk/threads/105676/"')
        end

        if imgui.Button(fa.ICON_SITEMAP .. u8" Тема на BlastHack", imgui.ImVec2(150,25)) then
            os.execute('explorer "https://www.blast.hk/threads/105676/"')
        end

        imgui.SameLine()
        if imgui.Button(fa.ICON_PAPER_PLANE .. u8" Тех.Поддержка", imgui.ImVec2(150,25)) then
            os.execute('explorer "https://vk.com/im?sel=-208243482"')
        end

        imgui.SameLine()
        if imgui.Button(fa.ICON_VK .. u8" Группа ВК", imgui.ImVec2(150,25)) then
